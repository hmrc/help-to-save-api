...
trait HelpToSaveApiService {
  ...
  def createAccountUserRestricted(request: Request[AnyContent], retrievedUserDetails: RetrievedUserDetails)(
...
class HelpToSaveApiServiceImpl @Inject() (
  ...
  private def storeEmailThenCreateAccount(json: JsValue, retrievedNINO: Option[String], request: Request[_])(
    implicit hc: HeaderCarrier,
    ec: ExecutionContext
  ): CreateAccountResponseType =
