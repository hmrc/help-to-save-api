...
trait HelpToSaveApiService {
  ...
  def createAccountPrivileged(
    request: Request[AnyContent]
  )(implicit hc: HeaderCarrier, ec: ExecutionContext): CreateAccountResponseType

  def createAccountUserRestricted(request: Request[AnyContent], retrievedUserDetails: RetrievedUserDetails)(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
...
@Singleton
class HelpToSaveApiServiceImpl @Inject() (
...
)(implicit config: Configuration, logMessageTransformer: LogMessageTransformer)
  ...
  override def createAccountUserRestricted(
    request: Request[AnyContent],
    retrievedUserDetails: RetrievedUserDetails
  )(implicit hc: HeaderCarrier, ec: ExecutionContext): CreateAccountResponseType = { // scalastyle:ignore
  ...
  override def createAccountPrivileged(
    request: Request[AnyContent]
  )(implicit hc: HeaderCarrier, ec: ExecutionContext): CreateAccountResponseType = { // scalastyle:ignore
  ...
  private def storeEmailThenCreateAccount(json: JsValue, retrievedNINO: Option[String], request: Request[_])(implicit
    ...
    validateCreateAccountRequest(json, request) { case CreateAccountRequest(header, body) =>
      if (retrievedNINO.forall(_ === body.nino)) {
        ...
        logger.warn(
          "Received create account request where NINO in request body did not match NINO retrieved from auth",
  ...
  private def createAccount(
    ...
    eligibilityResponse: ApiEligibilityResponse
          ...
          case Status.BAD_REQUEST =>
            ...
            val error = response
              ...
              .fold(
                ...
                e => ApiValidationError(e.errorDetail)
              )
  ...
  private def performCheckEligibility(
    nino: String,
    correlationId: UUID,
  ...
  private def storeEligibilityResultInMongo(
  ...
  private def validateCorrelationId(cId: UUID, nino: String)(implicit
    ec: ExecutionContext
  ...
  private def validateBankDetails(
    nino: NINO,
